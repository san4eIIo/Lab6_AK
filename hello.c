...
MODULE_PARM_DESC(param, "Num of times Hello, World will be printed");
...
static int __init hello_init(void)
{
    if (param == 0 || (param >= 5 && param <= 10)) {
        printk(KERN_WARNING "Param is 0 or 5< Param <10");  
    }

    if (param > 10) {
        printk(KERN_ERR "Too big param");
        return -EINVAL;
    }
